...
# Wichtig: Script in den Speicher laden damit kein fd-Problem entsteht
# wenn pct/ssh den stdin-Dateideskriptor erbt
if [[ "${TRMM_LOADED:-}" != "1" ]]; then
  export TRMM_LOADED=1
  exec bash -c "$(cat "$0")" -- "$@"
fi
...
_pick_storage() {
  ...
  if [[ ${#STORAGES[@]} -eq 0 ]]; then
    msg_error "Kein Storage gefunden!"
...
fetch_template() {
  ...
  if [[ -z "$tmpl_name" ]]; then
    msg_error "Kein Debian ${DEBIAN_VERSION} Template im Proxmox-Katalog gefunden!"
  fi
...
# --- Installations-Script fuer den Container erstellen -----------------------
# Wichtig: Kein pct exec mit mehrzeiligem bash -c String!
# Stattdessen: Script-Datei lokal schreiben, per pct push in Container,
# dort ausfuehren. Kein fd-Problem moeglich.
write_setup_script() {
...
# Pakete installieren
apt-get update -qq
apt-get install -yq curl wget sudo ufw git htop ncdu openssh-server
...
# TRMM Installationsscript
curl -fsSL https://raw.githubusercontent.com/amidaware/tacticalrmm/master/install.sh \
  -o /home/tactical/install.sh || true
chown tactical:tactical /home/tactical/install.sh 2>/dev/null || true
chmod +x /home/tactical/install.sh 2>/dev/null || true

# Hilfstools
curl -fsSL ${GITHUB_RAW}/scripts/trmm-update.sh -o /usr/local/bin/trmm-update || true
curl -fsSL ${GITHUB_RAW}/scripts/trmm-backup.sh -o /usr/local/bin/trmm-backup || true
chmod +x /usr/local/bin/trmm-update /usr/local/bin/trmm-backup 2>/dev/null || true
...
cat > /etc/motd << 'MOTDEOF'
  ...
  |                                                              |
  |  Schritt 2: Als tactical einloggen und starten:              |
  |    su - tactical                                             |
  |    ./install.sh                                              |
  |                                                              |
  |  Hilfstools:                                                 |
  |    trmm-update   TRMM aktualisieren                          |
  |    trmm-backup   Manuelles Backup                            |
  |                                                              |
...
mode_install() {
  ...
  if [[ "$RAM" -lt 4096 ]] 2>/dev/null; then
    msg_warn "Mindestens 4096 MB empfohlen."
...
_do_create() {
  ...
  pct start "$CTID"
  local retries=0
  until pct exec "$CTID" -- true </dev/null 2>/dev/null; do
    sleep 2
    ((retries++))
    [[ $retries -gt 15 ]] && msg_error "Container antwortet nicht!"
  done
  msg_ok "Container gestartet"

  msg_info "Kopiere Setup-Script in Container"
  pct push "$CTID" "${TEMP_DIR}/setup.sh" /root/setup.sh
  msg_ok "Setup-Script bereit"

  msg_info "Fuehre Setup aus (Pakete, User, TRMM-Script, ...)"
  pct exec "$CTID" -- bash /root/setup.sh </dev/null \
    || msg_error "Setup fehlgeschlagen - Logs: pct exec ${CTID} -- cat /var/log/setup.log"
  msg_ok "Setup abgeschlossen"

  msg_info "Bereinige temporaere Dateien im Container"
  pct exec "$CTID" -- rm -f /root/setup.sh </dev/null 2>/dev/null || true
  msg_ok "Bereinigt"
...
# --- Modus: Update ------------------------------------------------------------
write_update_script() {
  cat > "${TEMP_DIR}/run-update.sh" << UPDATEEOF
#!/bin/bash
curl -fsSL ${GITHUB_RAW}/scripts/trmm-update.sh | bash
UPDATEEOF
  chmod +x "${TEMP_DIR}/run-update.sh"
}
...
mode_update() {
  ...
  if [[ ${#TRMM_CTS[@]} -eq 0 ]]; then
    msg_warn "Kein Container mit 'tactical' oder 'trmm' gefunden."
  ...
  if [[ "$ct_status" != "running" ]]; then
    msg_warn "Container ${TARGET_CTID} laeuft nicht."
    echo -ne " Jetzt starten? [J/n]: "
    read -r s; s="${s:-J}"
    if [[ "${s,,}" != "n" ]]; then
      msg_info "Starte Container"
  ...
  write_update_script
  msg_info "Kopiere Update-Script"
  pct push "$TARGET_CTID" "${TEMP_DIR}/run-update.sh" /root/run-update.sh

  msg_info "Starte Update"
  pct exec "$TARGET_CTID" -- bash /root/run-update.sh </dev/null \
    || msg_error "Update fehlgeschlagen!"
  pct exec "$TARGET_CTID" -- rm -f /root/run-update.sh </dev/null 2>/dev/null || true
...
mode_backup() {
  ...
  echo ""
  echo -ne " Backup starten? [J/n]: "
  read -r confirm; confirm="${confirm:-J}"
  [[ "${confirm,,}" == "n" ]] && msg_error "Abgebrochen."

  msg_info "Erstelle CT-Snapshot"
  if pct snapshot "$TARGET_CTID" "$snap_name" \
      --description "Tactical RMM Backup ${ts}" 2>/dev/null; then
    msg_ok "Snapshot: ${snap_name}"
  else
    msg_warn "Snapshot fehlgeschlagen - weiter mit App-Backup."
  fi
  ...
  if [[ "$ct_status" == "running" ]]; then
    ...
    pct exec "$TARGET_CTID" -- bash -c \
      "command -v trmm-backup && trmm-backup --auto || true" \
      </dev/null 2>/dev/null \
      && msg_ok "App-Backup abgeschlossen" \
  ...
  echo ""
  echo -e " ${GN}Snapshots fuer CT ${TARGET_CTID}:${CL}"
...
select_mode() {
  ...
  divider
  echo ""
  echo -e "  ${BOLD}1)${CL} ${GN}Neuen LXC Container erstellen${CL}  - Debian + TRMM-Vorbereitung"
