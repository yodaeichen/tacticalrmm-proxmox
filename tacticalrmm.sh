#!/usr/bin/env bash
# =============================================================================
#  Proxmox VM Installer fuer Tactical RMM  (Community Script)
...
#
#  WICHTIG: Tactical RMM laeuft NUR auf VMs (kein LXC)
#  Quelle: https://docs.tacticalrmm.com/unsupported_guidelines/
# =============================================================================

set -eo pipefail

# --- Farben mit $'...' - funktioniert in read -p zuverlaessig -----------------
YW=$'\033[33m'
BL=$'\033[36m'
RD=$'\033[01;31m'
GN=$'\033[1;92m'
...
VLAN=""
START_VM="yes"
STORAGE_VM=""      # Storage fuer VM-Disk (muss images unterstuetzen)
STORAGE_SNIPPETS="" # Storage fuer Cloud-Init Snippets (optional)
TACTICAL_PASS=""
TARGET_VMID=""
SSH_USER="tactical"
...
prompt() {
  ...
  local input
  read -r input
  # Ergebnis in die gewuenschte globale Variable schreiben
...
BANNER
  echo -e "${CL}"
  echo -e "  ${BOLD}Proxmox VM Installer fuer Tactical RMM${CL}"
  echo -e "  ${DIM}Community Script - kein offizielles AmidaWare-Projekt${CL}"
  echo ""
}

# --- Proxmox-Voraussetzungen pruefen -------------------------------------------
check_proxmox() {
...
select_mode() {
  ...
  case "$MODE" in
    ...
    3) mode_backup ;;
    4) echo -e "\n Auf Wiedersehen!\n"; exit 0 ;;
    *) msg_warn "Ungueltige Auswahl - bitte 1, 2, 3 oder 4 eingeben."; select_mode ;;
  esac
}

# --- OS-Version auswaehlen -----------------------------------------------------
select_os_version() {
  echo ""
  ...
  echo -e "     ${DIM}Noch NICHT offiziell von Tactical RMM unterstuetzt.${CL}"
  echo -e "     ${DIM}Python 3.13 / PostgreSQL 17 koennen Probleme verursachen.${CL}"
  echo -e "     ${DIM}Nur fuer Tests - nicht fuer Produktion empfohlen!${CL}"
  echo ""
  ...
  case "$os_choice" in
    2)
      ...
      echo ""
      echo -e " ${RD}${BOLD}ACHTUNG:${CL} ${YW}Debian 13 ist von Tactical RMM offiziell NICHT unterstuetzt.${CL}"
...
_get_storages_with_content() {
  # Filtert Storages nach unterstuetztem Content-Typ
...
_pick_storage() {
  # Zeigt eine nummerierte Liste und laesst den User waehlen
...
# --- Storage-Auswahl fuer VM-Disk ----------------------------------------------
select_storage_vm() {
  echo ""
  echo -e " ${DIM}Storage fuer die VM-Disk (muss 'images' unterstuetzen):${CL}"
  _pick_storage "VM-Disk Storage" STORAGE_VM "images"
}

# --- Storage-Auswahl fuer Cloud-Init Snippets (optional) -----------------------
select_storage_snippets() {
  echo ""
  # Pruefen ob Snippets-faehige Storages existieren
  ...
  echo -e " ${DIM}Storage fuer Cloud-Init Snippets (fuer MOTD und Erstkonfiguration):${CL}"
...
mode_install() {
  ...
  # Storage fuer VM-Disk
  select_storage_vm

  # Storage fuer Cloud-Init Snippets
  select_storage_snippets
  ...
  if [[ "$DEBIAN_VERSION" == "13" ]]; then
    echo ""
    echo -e "  ${YW}! Debian 13 ist von Tactical RMM nicht offiziell unterstuetzt!${CL}"
  fi
  echo ""
...
CLOUDINIT
  ...
  echo ""
  divider
  echo -e " ${GN}${BOLD}OK VM erfolgreich erstellt!${CL}"
  divider
  echo ""
  printf "  %-16s ${BOLD}%s${CL}\n" "VM-ID:"    "$VMID"
  printf "  %-16s ${BOLD}%s${CL}\n" "OS:"       "Debian ${DEBIAN_VERSION} (${DEBIAN_CODENAME})"
...
mode_update() {
  ...
  divider
  echo -e " ${DIM}Fuehrt das offizielle Update-Script auf der TRMM-VM aus.${CL}"
  echo ""
  ...
  if [[ "$vm_status" != "running" ]]; then
    msg_warn "VM ${TARGET_VMID} laeuft nicht (Status: ${vm_status})"
  ...
  echo ""
  echo -e " ${INFO} Verbinde mit ${VM_IP} und fuehre Update aus..."
...
mode_backup() {
  ...
  if qm snapshot "$TARGET_VMID" "$snap_name" \
    ...
    msg_warn "Snapshot fehlgeschlagen (Storage unterstuetzt keine Snapshots?). Weiter mit App-Backup."
  fi
  ...
  if [[ -n "$VM_IP" ]]; then
    ...
    msg_warn "IP nicht ermittelbar - App-Backup uebersprungen."
  fi

  echo ""
  echo -e " ${GN}Vorhandene Snapshots fuer VM ${TARGET_VMID}:${CL}"
